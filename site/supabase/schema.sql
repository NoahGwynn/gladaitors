...
-- v3 utility pivot — decision output synthesis (Notion Phase 3).
-- After a debate completes, the user can generate a structured
-- decision-support synthesis: what each side surfaced, what would
-- change your mind, and an optional record of what you actually
-- decided. Turns "a debate you watched" into "a debate that helped
-- you decide".
--
-- Stored as three separate columns:
--   decision_synthesis        — JSON: {eachSideSurfaced: {side1: [], side2: []}, tensions: [], summary}
--                                Generated by a Sonnet-class model in a single
--                                post-debate call. Read-only once generated.
--   what_would_change_my_mind — TEXT: user-authored answer to the prompt.
--                                 Optional, editable by the owner.
--   user_decision             — JSON: {decision: string, rationale: string, timestamp}
--                                 Optional capture of what the user actually
--                                 decided after running the debate.
alter table public.debates add column if not exists decision_synthesis jsonb;
alter table public.debates add column if not exists what_would_change_my_mind text;
alter table public.debates add column if not exists user_decision jsonb;

-- Which template (if any) the debate was created from. NULL for
-- free-form debates. Enables per-template analytics and lets the
-- UI show the template name in the debate header.
alter table public.debates add column if not exists template_slug text;
