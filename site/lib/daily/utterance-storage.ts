// ============================================================================
// the dAIly — Utterance Storage (tagged memory writer)
// ============================================================================
// Persists a cast member's spoken response to forum_utterances with:
// - Embedding via text-embedding-3-small (reuses lib/daily/embed.ts)
...
async function tagUtterance(
  ...
  const prompt = `You are a topic classifier for the the dAIly, a daily AI investigation forum. A cast member has just spoken in a debate session for the ${category.toUpperCase()} category. Assign 1-3 topic tags from the fixed taxonomy below that best describe what their statement is about.
