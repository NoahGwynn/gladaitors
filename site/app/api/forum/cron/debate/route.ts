// ============================================================================
// POST /api/forum/cron/debate — Stage 4 of the daily cron chain
// ============================================================================
...
export async function POST(request: NextRequest) {
  ...
  if (session.status !== 'agenda_built') {
    return Response.json(
      {
        error: `Session status is '${session.status}' — must be 'agenda_built' to run the debate. Run /api/forum/cron/prepare first.`,
      },
      { status: 400 },
    );
  }
