// ============================================================================
// POST /api/forum/cron/prepare — Stage 3 of the daily cron chain
// ============================================================================
...
import type { MergedShortlistEntry, OrganizeResult } from '@/lib/forum/organize';
...
export async function POST(request: NextRequest) {
  ...
  if (loadErr || !sessionData) {
    return Response.json({
      error: `No session row for ${category} ${sessionDate}. Run /api/forum/cron/create-session or /api/forum/cron/organize first.`,
    }, { status: 404 });
  }
  ...
  if (!session.organize_snapshot) {
    return Response.json({
      error: 'organize_snapshot missing — run /api/forum/cron/organize first. Prepare does not run organize itself.',
    }, { status: 409 });
  }
